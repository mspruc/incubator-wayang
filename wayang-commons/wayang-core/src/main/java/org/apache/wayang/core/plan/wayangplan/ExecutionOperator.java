...
public interface ExecutionOperator extends ElementaryOperator {
    ...
    /**
     * Creates a {@link LoadProfileEstimator} according to the
     * {@link Configuration}.
     *
     * @param configuration     the {@link Configuration}
     * @param configurationKeys keys for the specification within the
     *                          {@link Configuration}
     * @return the {@link LoadProfileEstimator} or {@code null} if none could be
     *         created
     */
    static LoadProfileEstimator createLoadProfileEstimators(final Configuration configuration,
            final Collection<String> configurationKeys) {
        LoadProfileEstimator mainEstimator = null;
        for (final String configurationKey : configurationKeys) {
            final LoadProfileEstimator loadProfileEstimator = LoadProfileEstimators
                    .createFromSpecification(configurationKey, configuration);
    ...
    /**
     * Models eager execution by marking all {@link LazyExecutionLineageNode}s as
     * executed and collecting all marked ones.
     *
     * @param inputs          the input {@link ChannelInstance}s
     * @param outputs         the output {@link ChannelInstance}s
     * @param operatorContext the executed
     *                        {@link OptimizationContext.OperatorContext}
     * @return the executed {@link OptimizationContext.OperatorContext} and produced
     ...
     *         {@link ChannelInstance}s
     */
    static Tuple<Collection<ExecutionLineageNode>, Collection<ChannelInstance>> modelLazyExecution(
            final ChannelInstance[] inputs,
            final ChannelInstance[] outputs,
            final OptimizationContext.OperatorContext operatorContext) {
    ...
    /**
     * @return this instance or, if it was derived via {@link #copy()}, the original
     *         instance
     */
    ExecutionOperator getOriginal();

    /**
     * Developers of {@link ExecutionOperator}s can provide a default
     * {@link LoadProfileEstimator} via this method.
     *
     * @param configuration in which the {@link LoadProfile} should be estimated.
     * @return an {@link Optional} that might contain the
     *         {@link LoadProfileEstimator} (but {@link Optional#empty()}
     *         by default)
     */
    default Optional<LoadProfileEstimator> createLoadProfileEstimator(final Configuration configuration) {
        final Collection<String> configurationKeys = this.getLoadProfileEstimatorConfigurationKeys();
        final LoadProfileEstimator mainEstimator = createLoadProfileEstimators(configuration, configurationKeys);
        return Optional.ofNullable(mainEstimator);
    }

    /**
     * Provide the {@link Configuration} keys for the {@link LoadProfileEstimator}
    ...
    default Collection<String> getLoadProfileEstimatorConfigurationKeys() {
        ...
        return singleKey == null ? Collections.emptyList() : Collections.singletonList(singleKey);
    }
    ...
    default String getLimitBaseKey() {
        ...
        final String loadProfileEstimatorConfigurationKey = this.getLoadProfileEstimatorConfigurationKey();
     ...
     *
     * @param operatorContext an {@link OptimizationContext.OperatorContext} within
     *                        which this instance might be
     *                        executed
     * @return whether this instance should <b>not</b> be used for execution
     */
    default boolean isFiltered(final OptimizationContext.OperatorContext operatorContext) {
        ...
        final String limitBaseKey = this.getLimitBaseKey();
        if (limitBaseKey != null) {
            final Configuration configuration = operatorContext.getOptimizationContext().getConfiguration();

            // Check the inputs.
            for (final InputSlot<?> input : this.getAllInputs()) {
                final String key = limitBaseKey + "." + input.getName();
                final long limit = configuration.getLongProperty(key, -1);
                if (limit >= 0) {
                    final CardinalityEstimate cardinality = operatorContext.getInputCardinality(input.getIndex());
                    if (cardinality != null && cardinality.getGeometricMeanEstimate() > limit)
                        return true;
                }
            }

            // Check the outputs.
            for (final OutputSlot<?> output : this.getAllOutputs()) {
                final String key = limitBaseKey + "." + output.getName();
                final long limit = configuration.getLongProperty(key, -1);
                if (limit >= 0) {
                    final CardinalityEstimate cardinality = operatorContext.getOutputCardinality(output.getIndex());
                    if (cardinality != null && cardinality.getGeometricMeanEstimate() > limit)
                        return true;
                }
            }
     ...
     * @return an {@link List} of {@link Channel}s' {@link Class}es, ordered by
     ...
     * @deprecated {@link ExecutionOperator}s should only support a single
     ...
     */
    default ChannelDescriptor getOutputChannelDescriptor(final int index) {
     ...
     *
     * @param task                    the {@link ExecutionTask} in which this
     *                                instance is being wrapped
     * @param producerOperatorContext the
     *                                {@link OptimizationContext.OperatorContext}
     *                                for this instance
     * @param inputChannelInstances   the input {@link ChannelInstance}s for the
     *                                {@code task}
     * @return
     */
    default ChannelInstance[] createOutputChannelInstances(final Executor executor,
        ...
        final ChannelInstance[] channelInstances = new ChannelInstance[task.getNumOuputChannels()];
        for (int outputIndex = 0; outputIndex < channelInstances.length; outputIndex++) {
            ...
            final ChannelInstance outputChannelInstance = outputChannel.createInstance(executor,
                    producerOperatorContext, outputIndex);
