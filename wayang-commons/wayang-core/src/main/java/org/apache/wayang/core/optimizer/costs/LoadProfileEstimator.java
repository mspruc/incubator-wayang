...
/**
 * Estimates the {@link LoadProfile} of some executable artifact that takes some
 * input data quanta and produces them.
 */
public interface LoadProfileEstimator extends Serializable {
    ...
    default Collection<String> getConfigurationKeys() {
        ...
        for (final LoadProfileEstimator nestedEstimator : this.getNestedEstimators()) {
    ...
    /**
     * Retrieve the {@link Configuration} template key if any. Usually, this is the
     * {@link Configuration} key
    ...
    /**
     * Retrieve the {@link Configuration} template keys for this and all nested
     * instances.
     * 
     * @return the template keys (no {@code nulls})
     */
    default Collection<String> getTemplateKeys() {
        final Collection<String> templateKeys = new LinkedList<>();
