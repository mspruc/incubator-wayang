 ...
 */
public class FlinkTsvFileSink<T extends Tuple2<?, ?>> extends UnarySink<T> implements FlinkExecutionOperator {

    private final String targetPath;

    public FlinkTsvFileSink(final DataSetType<T> type) {
        this(null, type);
    }

    public FlinkTsvFileSink(final String targetPath, final DataSetType<T> type) {
        super(type);
    ...
    public Tuple<Collection<ExecutionLineageNode>, Collection<ChannelInstance>> evaluate(
        ...
        final String path = output.addGivenOrTempPath(this.targetPath, flinkExecutor.getConfiguration());

        final DataSetChannel.Instance input = (DataSetChannel.Instance) inputs[0];

        final DataSet<String> map = input.<T>provideDataSet()
                .map(new MapFunction<T, String>() {
                    @Override
                    public String map(final Tuple2 dataQuantum) throws Exception {
                        final Tuple2<?,?> tuple2 = dataQuantum;
                        return String.valueOf(tuple2.getField0()) + '\t' + String.valueOf(tuple2.getField1());
                    }
                }).setParallelism(flinkExecutor.fee.getParallelism());

        map.writeAsText(path)
                .setParallelism(flinkExecutor.fee.getParallelism());
